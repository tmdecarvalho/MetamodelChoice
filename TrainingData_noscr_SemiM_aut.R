...
# THIS FILE DOES...
# analysis type 1: build training data for emulator, by running simulation multiple times at DIFFERENT parameter values ;


#packages
#library(xlsx)  # note: read.table is not working for xlsx 
library(lhs)
library(DescTools)
...
#from this point on pretty much copy/paste loop 1 from SimulationModel_v8              ******************************
#         *********************************
...
for (s in c(1:S) ) {
  ...
  # COPY/PASTE from SimulationModel_v5 (or posterior versions)       #&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&& &
  
  # DO NOT MAKE ANY CHANGES HERE, OTHERWISE RESULTS COULD BECOME INCONSISTENT
  # if changes are needed:
  # 1) change in the original file (SimulationModel_v5)
  # 2) copy/paste here
  ...
  for (i in 1:M)   { 
    ...
    while (!skiptransitions) { # if natural history is not yet finished 
      ...
      duration[trans] <- rGompertz(1,alpha*times[trans]/time.followup, exp(hazm[trans,trans+1]+riskcov1[trans,trans+1]*pop_cat+
                                                                             riskcov2[trans,trans+1]*pop_cat2 +agecov[trans,trans+1]*(age0-centerage))  ) 
